...
| **Verification** | Window product-proof tests; batteries; `verify-observation-answer`; `verify-application-identity`; `tests/observation-answer.test.ts`; `tests/application-identity.test.ts`; kernel `application_identity_tests` |
| **Product Proof** | “What windows are open?” live path — Owner Product Proof still required for the P23.S4 answer form and the P23.S6 observation data |
| **Priority** | — |
| **Engineering Notes** | Title match — not tab/DOM. P23.S4 reuses this observation unchanged as the first answer source at the `capability-observation` rung: the comprehended need reaches the existing `window/enumerate` request, and the returned `items` are composed into a conversational answer. No second enumeration, no new Kernel interface. P23.S6 enriched the observation without touching the capability: `ApplicationWindowItem` now carries `process_name`, the executable image basename Windows already reported during capture and the item silently dropped. The inventory answer still lists titles — that is what the Owner asked for — and names the application only to separate windows whose titles collide. |
...
| **Layer** | L4 Observation |
| **Status** | **IMPLEMENTED** |
| **Lifecycle** | Engineering complete — P23.S5 / P23.S6 answer forms awaiting Owner Product Proof |
| **Dependencies** | Window Provider `active` |
| **Verification** | Window provider / intent map `winActive`; `verify-observation-answer`; `verify-application-identity`; `tests/active-window-answer.test.ts`; `tests/application-identity.test.ts`; kernel `application_identity_tests` |
| **Product Proof** | “Which window is active?” and “Which application am I using?” live paths — Owner Product Proof required for the P23.S5 / P23.S6 answer forms |
| **Priority** | Architecture slices P23.S5, P23.S6 |
| **Engineering Notes** | Foreground window truth. P23.S5 reuses this observation unchanged as the second answer source at the `capability-observation` rung: the comprehended need reaches the existing `window/active` request, and the returned focused item is composed into a conversational answer. No second observation, no new Kernel interface. P23.S6 retired the identity deficit both slices hit: the item now carries `process_name`, so “which application” is answered from observation while “which window” stays a window answer. The application is reported exactly as Windows names the executable (`Code.exe`) — no case fixing, no extension stripping, no product-name lookup, because every such transformation is a claim the observation does not support. When the OS names no application, which it does for protected processes, Conversation says so instead of reading one out of the title. |
