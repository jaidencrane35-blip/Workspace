...
goals, or intents.
`DecisionCandidateProgressionAcknowledgement` then records DE-side receipt of that
request for a future workflow — still without planner, Gateway, goals, or intents.
See
...
and
[WORKSPACE-DECISION-ENGINE-CANDIDATE-PROGRESSION-ACKNOWLEDGEMENT.md](./WORKSPACE-DECISION-ENGINE-CANDIDATE-PROGRESSION-ACKNOWLEDGEMENT.md).
