...
Application identity remains unresolved at this point and is visible in the product: asked "Which application am I using?", Workspace answers with the window title and says it cannot name the program. This is the second slice to hit the same missing field, which raises it from an inconvenience to a real dependency — `ApplicationWindowItem` carries no `process_name`, and until it does, every answer about *what the Owner is using* is a title. (Retired by P23.S6, below.)
...
### Status — P23.S6 delivered (the application is observed, not deduced)

Two slices independently stopped at the same sentence: *I can see the window, but not which program owns it.* P23.S6 retired that dependency, and the audit explains why it was never a missing capability. Windows already reported the owning application on **every** window of **every** capture — `win32.rs` calls `QueryFullProcessImageNameW` under `PROCESS_QUERY_LIMITED_INFORMATION` and stores the executable image basename on `CapturedDesktopWindow`. The value then died in the conversion to `DesktopWindowSnapshot`, which did not declare the field. Nothing above that line could name an application because nothing below it passed one up.

So this slice added no observation, no provider, no IPC channel, and no Windows API. It widened the snapshot and `ApplicationWindowItem` by one optional field and removed two hand-written copies of `to_legacy_snapshot` in the Window Provider — duplication that is precisely how a field goes missing in one code path and not another.

**What "application identity" means here** is fixed deliberately narrowly: the executable image basename, reported exactly as Windows spells it (`Code.exe`). It is *not* a product name. Workspace does not strip the extension, correct the case, or map the executable to a friendlier label, because each of those is a claim the observation does not support — and the version-resource `FileDescription` that would supply a genuine product name is a separate authority with its own failure modes. Presenting the OS value verbatim also keeps the evidence chain checkable end to end: the string in the answer is the string Windows produced.

**Absence is information.** The field is optional the whole way down, because Windows genuinely withholds it for protected and elevated processes. An unreported application produces "I can see the window, but Windows didn't tell me which application is behind it" — never a name recovered from the title. The tests fail if identity is deleted from the observation and an answer still claims one, and the verifier fails if the answer source acquires an application name of its own or derives one from a title.

**Windows and applications stayed different questions.** "Which window is active?" answers with the window; "Which application am I using?" answers with the application. The inventory answer still lists titles alone — the Owner asked what is open, and adding an executable to every line would cost verbosity for information nobody requested — and names applications only to separate windows whose titles collide.

**One substitution was closed on the way.** "What application is active?" was being handed to ChatGPT: comprehension classified it as machine perception, but the intelligence cascade had already resolved an external handoff, and the observation bridge refused to override any resolved action. The bridge now replaces two kinds of action and no others — one that merely speaks, and one that hands the question to an external model — because neither is a desktop goal, and the ladder already ranks an authorized observation above an external source. **A question Workspace can observe must never be sent to another AI.** Anything that acts on the desktop remains untouchable.
