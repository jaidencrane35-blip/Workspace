...
import {
  isEvidenceTraceHistoryNonActionable,
...
import {
  isEvidenceDependencyHistoryNonActionable,
...
import {
  isEvidenceCompletenessHistoryNonActionable,
...
describe("projection integrity — cognitive agent cast", () => {
  ...
  it("keeps evidence trace projections non-commandable with authoritative history_count", () => {
    const historyEntry = (
      overrides: Partial<EvidenceTraceHistoryEntry> = {}
    ): EvidenceTraceHistoryEntry => ({
    ...
    const projection: WorkspaceEvidenceTraceProjection = {
      workspace_id: "ws",
      projected_at: "t2",
      current: null,
      history: [historyEntry()],
    ...
    expect(isEvidenceTraceProjectionNonCommandable(projection)).toBe(true);
    expect(isEvidenceTraceHistoryNonActionable(historyEntry())).toBe(true);
    expect(
      isEvidenceTraceHistoryNonActionable(historyEntry({ actionable: true }))
    ).toBe(false);

    const summary: WorkspaceEvidenceTraceSummary = {
      workspace_id: "ws",
      has_current: false,
      completeness: null,
      segment_count: 0,
    ...
    expect(evidenceTraceHistoryCountIsAuthoritative(summary)).toBe(true);
    expect(summary.history_count).toBeGreaterThanOrEqual(summary.history.length);
  ...
  it("keeps evidence dependency projections non-commandable with authoritative history_count", () => {
    const historyEntry = (
      overrides: Partial<EvidenceDependencyHistoryEntry> = {}
    ): EvidenceDependencyHistoryEntry => ({
      dependency_id: "evidence_dependency:1",
      status: "superseded",
      created_at: "t0",
      superseded_at: "t1",
      completeness: "partial",
    ...
    const projection: WorkspaceEvidenceDependencyProjection = {
      workspace_id: "ws",
      projected_at: "t2",
      current: null,
      history: [historyEntry()],
    ...
    expect(isEvidenceDependencyProjectionNonCommandable(projection)).toBe(true);
    expect(isEvidenceDependencyHistoryNonActionable(historyEntry())).toBe(true);
    expect(
      isEvidenceDependencyHistoryNonActionable(historyEntry({ actionable: true }))
    ).toBe(false);

    const summary: WorkspaceEvidenceDependencySummary = {
      workspace_id: "ws",
      has_current: false,
      completeness: null,
      node_count: 0,
    ...
    expect(evidenceDependencyHistoryCountIsAuthoritative(summary)).toBe(true);
    expect(summary.history_count).toBeGreaterThanOrEqual(summary.history.length);
  ...
  it("keeps evidence completeness projections non-commandable with authoritative history_count", () => {
    const historyEntry = (
      overrides: Partial<EvidenceCompletenessHistoryEntry> = {}
    ): EvidenceCompletenessHistoryEntry => ({
    ...
    const projection: WorkspaceEvidenceCompletenessProjection = {
      workspace_id: "ws",
      projected_at: "t2",
      current: null,
      history: [historyEntry()],
    ...
    expect(isEvidenceCompletenessProjectionNonCommandable(projection)).toBe(true);
    expect(isEvidenceCompletenessHistoryNonActionable(historyEntry())).toBe(true);
    expect(
      isEvidenceCompletenessHistoryNonActionable(historyEntry({ actionable: true }))
    ).toBe(false);

    const summary: WorkspaceEvidenceCompletenessSummary = {
      workspace_id: "ws",
      has_current: false,
      completeness: null,
      observation_count: 0,
    ...
    expect(evidenceCompletenessHistoryCountIsAuthoritative(summary)).toBe(true);
    expect(summary.history_count).toBeGreaterThanOrEqual(summary.history.length);
