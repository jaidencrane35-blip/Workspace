...
use tauri::State;
use workspace_domain::{WorkspaceRuntimeState, WorkspaceState};
...
/// Returns the full live WorkspaceRuntimeState bundle (not Experience chrome).
#[tauri::command]
pub fn get_workspace_runtime_state(
    kernel: State<'_, Arc<Mutex<WorkspaceKernel>>>,
) -> IpcResponse<WorkspaceRuntimeState> {
    match kernel.lock() {
        Ok(kernel) => match CommandHandler::get_workspace_runtime_state(
            &kernel,
            ipc_actor_context(),
            ipc_intent_context(),
        ) {
