...
export default function App() {
  ...
  const refreshReads = useCallback(
    async (workspaceId: string) => {
      ...
      setZones(
        (nextContext.snapshot.zones as Array<{
          resource_ref?: { id?: string };
          name?: string;
        }> | undefined)?.map((z) => ({
          id: String(z.resource_ref?.id ?? ""),
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(async ([nextStatus, nextHealth, nextSettings]) => {
  ...
  return (
    <main className="container">
      <h1>Workspace</h1>
      <p className="lede">
        Operator console — create workspace, seed zones, approve suggestions,
        bridge intent, execute, inspect outcomes.
      </p>
      ...
      <section>
        ...
        {!lastIntent && !lastExecution && (
          <p className="muted">Accept a suggestion, then create intent / execute.</p>
        )}
      </section>
