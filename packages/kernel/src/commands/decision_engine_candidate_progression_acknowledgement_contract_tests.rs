//! DecisionCandidateProgressionAcknowledgement — receipt only
//! (not planner / Gateway / goals / intents / execution / RE mutation).
...
use workspace_domain::{
    ...
    DecisionCandidateLifecycleIntegration, DecisionCandidateProgressionAcknowledgement,
    DecisionCandidateProgressionAcknowledgementInput, DecisionCandidateProgressionRequest,
    DecisionCandidateProgressionRequestInput, DecisionCandidateRanking,
...
fn awaiting_ack_input(
    candidate: DecisionCandidate,
) -> DecisionCandidateProgressionAcknowledgementInput {
    ...
    let ranking_entry = ranking
        ...
        .cloned();
    let selection_input = DecisionCandidateSelectionInput {
        candidate: candidate.clone(),
        score: Some(score),
    ...
    let (selection, _) = DecisionCandidateSelection::try_select(
        &selection_input,
    ...
    .expect("select");
    let request_input = DecisionCandidateProgressionRequestInput {
        candidate: candidate.clone(),
        selection,
        lifecycle_integration: lifecycle_integration.clone(),
        intake_withdrawn_or_invalidated: false,
    ...
    let (progression_request, _) =
        DecisionCandidateProgressionRequest::try_request(&request_input, "downstream", "t-req")
            .expect("request");
    DecisionCandidateProgressionAcknowledgementInput {
        candidate,
        progression_request,
        ...
        intake_withdrawn_or_invalidated: false,
        existing_acknowledgement: None,
    }
}

/// CASE 1 — Valid progression request can be acknowledged.
#[test]
fn case1_valid_request_can_be_acknowledged() {
    let input = awaiting_ack_input(native_candidate_with_total("ack-1", 50));
    assert!(DecisionCandidateProgressionAcknowledgement::derive(&input).is_awaiting());
    let before = input.candidate.clone();
    let (ack, unchanged) = DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "received",
        "t-ack",
    )
    .expect("acknowledge");
    ...
    assert!(ack
        .acknowledgement_id
        .starts_with(DecisionCandidateProgressionAcknowledgement::ID_PREFIX));
    assert_eq!(unchanged.outcome, before.outcome);
    assert!(ack.assert_acknowledgement_only().is_ok());
}

/// CASE 2 — Invalid request cannot acknowledge.
#[test]
fn case2_invalid_request_cannot_acknowledge() {
    let mut input = awaiting_ack_input(native_candidate_with_total("ack-2", 50));
    input.progression_request.request_state =
        DecisionCandidateProgressionRequest::STATE_PENDING.into();
    input.progression_request.requested_at = None;
    assert!(DecisionCandidateProgressionAcknowledgement::derive(&input).is_rejected());
    assert!(DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "x",
        "t",
...
/// CASE 3 — Withdrawn candidate cannot acknowledge.
#[test]
fn case3_withdrawn_cannot_acknowledge() {
    let mut input = awaiting_ack_input(intake_created_candidate("ack-3"));
    input.intake_withdrawn_or_invalidated = true;
    assert!(DecisionCandidateProgressionAcknowledgement::derive(&input).is_expired());
    assert!(DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "x",
...
/// CASE 4 — Invalid provenance blocks acknowledgement.
#[test]
fn case4_invalid_provenance_blocks() {
    let mut input = awaiting_ack_input(intake_created_candidate("ack-4"));
...
/// CASE 5 — Acknowledgement does not create planner handoff.
#[test]
fn case5_no_planner_handoff() {
    let input = awaiting_ack_input(native_candidate_with_total("ack-5", 50));
    let (ack, _) = DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "received",
        "t-ack",
    ...
    .unwrap();
    assert!(!ack.planner_invoked && !ack.may_invoke_planner());
    assert!(ack.handoff_command.is_none());
    assert!(ack.attempt_invoke_planner().is_err());
}

/// CASE 6 — Acknowledgement does not create goals/intents.
#[test]
fn case6_no_goals_intents() {
    let input = awaiting_ack_input(native_candidate_with_total("ack-6", 50));
    let (ack, _) = DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "received",
    ...
    .unwrap();
    assert!(!ack.creates_goal && !ack.may_create_goal());
    assert!(!ack.creates_intent && !ack.may_create_intent());
    assert!(ack.attempt_create_goal().is_err());
    assert!(ack.attempt_create_intent().is_err());
}

/// CASE 7 — Acknowledgement does not call Gateway.
#[test]
fn case7_no_gateway() {
    let input = awaiting_ack_input(native_candidate_with_total("ack-7", 50));
    let (ack, _) = DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "received",
    ...
    .unwrap();
    assert!(!ack.may_invoke_gateway());
    assert!(ack.attempt_invoke_gateway().is_err());
}

/// CASE 8 — RE overlays remain unchanged.
#[test]
fn case8_re_unchanged() {
    let input = awaiting_ack_input(intake_created_candidate("ack-8"));
    let (ack, _) = DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "received",
    ...
    .unwrap();
    assert!(!ack.mutates_recommendation_engine);
    assert!(!ack.ownership_transferred);
}

/// CASE 9 — Existing DecisionCandidate lifecycle remains unchanged.
#[test]
fn case9_candidate_lifecycle_unchanged() {
    let input = awaiting_ack_input(native_candidate_with_total("ack-9", 50));
    let before = input.candidate.outcome;
    let (ack, unchanged) = DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "received",
    ...
    .unwrap();
    assert!(!ack.mutates_candidate_outcome);
    assert_eq!(unchanged.outcome, before);
    assert_eq!(unchanged.outcome, DecisionOutcome::Open);
}

/// CASE 10 — Existing synthesis remains unchanged.
#[test]
fn case10_synthesis_unchanged() {
    let native = native_candidate_with_total("ack-10", 10);
    ...
    let input = awaiting_ack_input(native);
    let (ack, unchanged) = DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "received",
    ...
    .unwrap();
    let with = base.with_progression_acknowledgements(vec![ack]);
    assert_eq!(with.candidates[0].score.total, score_before);
    assert_eq!(with.candidates[0].outcome, DecisionOutcome::Open);
    assert_eq!(unchanged.outcome, DecisionOutcome::Open);
}

/// CASE 11 — Acknowledgement cannot silently execute.
#[test]
fn case11_cannot_silently_execute() {
    let input = awaiting_ack_input(native_candidate_with_total("ack-11", 50));
    let (ack, _) = DecisionCandidateProgressionAcknowledgement::try_acknowledge(
        &input,
        DecisionCandidateProgressionAcknowledgement::ACTION_ACKNOWLEDGE,
        "received",
    ...
    .unwrap();
    assert!(!ack.may_execute());
    assert_blocked(
        "execute",
        DecisionCandidateProgressionAcknowledgement::attempt_execute().map_err(KernelError::from),
    );
