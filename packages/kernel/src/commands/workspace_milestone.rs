...
/// Capability gate for Milestone Engine reads (coordination projection only).
/// Reuses existing `work_context.read` — no new Gateway paths.
pub struct GateMilestoneRead;

impl crate::commands::Command for GateMilestoneRead {
    fn name(&self) -> &'static str {
        "GateMilestoneRead"
    }
}

impl QueryCommand for GateMilestoneRead {
    type Output = ();
