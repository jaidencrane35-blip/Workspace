//! Programme IV Batch 7 — Workspace Evidence Freshness Engine contract tests.

use workspace_domain::{
    recovery_must_not_fabricate_evidence_freshness, ActorContext, EvidenceFreshnessCompleteness,
    FreshnessScope, IntentContext,
};
...
#[test]
fn evidence_freshness_is_non_executing() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let snap = CommandHandler::generate_workspace_evidence_freshness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws,
    ...
    assert!(recovery_must_not_fabricate_evidence_freshness(&snap));
    let current = snap.current.as_ref().expect("current evidence freshness");
...
fn unavailable_upstreams_preserved_as_gaps() {
    ...
    assert!(
        current.completeness == EvidenceFreshnessCompleteness::Unavailable
            || current.completeness == EvidenceFreshnessCompleteness::Partial
            || current.completeness == EvidenceFreshnessCompleteness::Unknown
            || current.completeness == EvidenceFreshnessCompleteness::Contradictory
    );
...
#[test]
fn evidence_freshness_recomputation_supersedes_history() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let actor = ActorContext::local_user();
    let intent = IntentContext::user_request();
    let first = CommandHandler::generate_workspace_evidence_freshness(
        &kernel,
        actor.clone(),
        intent.clone(),
        ws.clone(),
    ...
    .unwrap();
    let first_id = first.current.as_ref().unwrap().freshness_id.clone();
    let second = CommandHandler::generate_workspace_evidence_freshness(
        &kernel, actor, intent, ws,
    )
    .unwrap();
    assert_ne!(
        second.current.as_ref().unwrap().freshness_id,
        first_id
    );
...
#[test]
fn evidence_freshness_restart_continuity_without_fabricating() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let generated = CommandHandler::generate_workspace_evidence_freshness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws.clone(),
    ...
    .unwrap();
    let loaded = CommandHandler::get_workspace_evidence_freshness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws,
    ...
    .unwrap();
    assert_eq!(
        loaded.current.as_ref().map(|c| &c.freshness_id),
        generated.current.as_ref().map(|c| &c.freshness_id)
    );
    assert!(recovery_must_not_fabricate_evidence_freshness(&loaded));
}

#[test]
fn empty_evidence_freshness_remains_missing_until_generated() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    let snap = CommandHandler::get_workspace_evidence_freshness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws,
...
#[test]
fn evidence_freshness_transaction_rollback_leaves_no_partial_write() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let actor = ActorContext::local_user();
    let intent = IntentContext::user_request();
    let before = CommandHandler::generate_workspace_evidence_freshness(
        &kernel,
        actor.clone(),
        intent.clone(),
        ws.clone(),
    ...
    .unwrap();
    WorkspaceEvidenceFreshnessService::generate_with_forced_rollback(
    ...
    .unwrap();
    let after =
        CommandHandler::get_workspace_evidence_freshness(&kernel, actor, intent, ws).unwrap();
    assert_eq!(
        before.current.as_ref().unwrap().freshness_id,
        after.current.as_ref().unwrap().freshness_id
    );
    assert_eq!(before.history_count, after.history_count);
}

#[test]
fn evidence_freshness_negative_authority_guards() {
    assert!(WorkspaceEvidenceFreshnessService::attempt_execute().is_err());
    assert!(WorkspaceEvidenceFreshnessService::attempt_create_recommendation().is_err());
    assert!(WorkspaceEvidenceFreshnessService::attempt_estimate_freshness().is_err());
    assert!(WorkspaceEvidenceFreshnessService::attempt_refresh_evidence().is_err());
    assert!(WorkspaceEvidenceFreshnessService::attempt_regenerate_snapshot().is_err());
    assert!(WorkspaceEvidenceFreshnessService::attempt_fabricate_timestamp().is_err());
    assert!(WorkspaceEvidenceFreshnessService::attempt_schedule_update().is_err());
    assert!(WorkspaceEvidenceFreshnessService::attempt_emit_command().is_err());
    assert!(CommandHandler::evidence_freshness_attempt_execute().is_err());
}

#[test]
fn evidence_freshness_capability_deny_without_write() {
    ...
    let err = crate::commands::pipeline::CommandPipeline::new(ctx)
        .execute_mutation(
            crate::commands::workspace_evidence_freshness::GenerateWorkspaceEvidenceFreshness::new(
                ws,
            ),
        )
...
#[test]
fn explain_evidence_freshness_is_non_actionable() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    ...
    .unwrap();
    let explanation = CommandHandler::explain_evidence_freshness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws,
...
#[test]
fn evidence_freshness_history_is_non_actionable_evidence() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let actor = ActorContext::local_user();
    ...
    let summary = CommandHandler::get_workspace_evidence_freshness_summary(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
...
#[test]
fn evidence_freshness_service_never_calls_foreign_generate() {
...
#[test]
fn no_execution_paths_in_evidence_freshness_service() {
    let source = include_str!("../services/workspace_evidence_freshness.rs");
...
#[test]
fn no_fabricated_freshness_without_upstream() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    let snap = CommandHandler::generate_workspace_evidence_freshness(
        &kernel,
...
#[test]
fn stale_unknown_unavailable_never_imply_refresh() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let snap = CommandHandler::generate_workspace_evidence_freshness(
    ...
    for obs in &current.observations {
        ...
        assert!(!lower.contains("must refresh"));
        assert!(!lower.contains("please refresh"));
        assert!(!lower.contains("regenerate now"));
