//! Programme IV Batch 8 — Workspace Evidence Completeness Engine contract tests.

use workspace_domain::{
    recovery_must_not_fabricate_evidence_completeness, ActorContext, EvidenceCompletenessCompleteness,
    CompletenessScope, IntentContext,
};
...
#[test]
fn evidence_completeness_is_non_executing() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let snap = CommandHandler::generate_workspace_evidence_completeness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws,
    ...
    assert!(recovery_must_not_fabricate_evidence_completeness(&snap));
    let current = snap.current.as_ref().expect("current evidence completeness");
...
fn unavailable_upstreams_preserved_as_gaps() {
    ...
    assert!(
        current.completeness == EvidenceCompletenessCompleteness::Unavailable
            || current.completeness == EvidenceCompletenessCompleteness::Partial
            || current.completeness == EvidenceCompletenessCompleteness::Unknown
            || current.completeness == EvidenceCompletenessCompleteness::Contradictory
    );
...
#[test]
fn evidence_completeness_recomputation_supersedes_history() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let actor = ActorContext::local_user();
    let intent = IntentContext::user_request();
    let first = CommandHandler::generate_workspace_evidence_completeness(
        &kernel,
        actor.clone(),
        intent.clone(),
        ws.clone(),
    ...
    .unwrap();
    let first_id = first.current.as_ref().unwrap().completeness_id.clone();
    let second = CommandHandler::generate_workspace_evidence_completeness(
        &kernel, actor, intent, ws,
    )
    .unwrap();
    assert_ne!(
        second.current.as_ref().unwrap().completeness_id,
        first_id
    );
...
#[test]
fn evidence_completeness_restart_continuity_without_fabricating() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let generated = CommandHandler::generate_workspace_evidence_completeness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws.clone(),
    ...
    .unwrap();
    let loaded = CommandHandler::get_workspace_evidence_completeness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws,
    ...
    .unwrap();
    assert_eq!(
        loaded.current.as_ref().map(|c| &c.completeness_id),
        generated.current.as_ref().map(|c| &c.completeness_id)
    );
    assert!(recovery_must_not_fabricate_evidence_completeness(&loaded));
}

#[test]
fn empty_evidence_completeness_remains_missing_until_generated() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    let snap = CommandHandler::get_workspace_evidence_completeness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws,
...
#[test]
fn evidence_completeness_transaction_rollback_leaves_no_partial_write() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let actor = ActorContext::local_user();
    let intent = IntentContext::user_request();
    let before = CommandHandler::generate_workspace_evidence_completeness(
        &kernel,
        actor.clone(),
        intent.clone(),
        ws.clone(),
    ...
    .unwrap();
    WorkspaceEvidenceCompletenessService::generate_with_forced_rollback(
    ...
    .unwrap();
    let after =
        CommandHandler::get_workspace_evidence_completeness(&kernel, actor, intent, ws).unwrap();
    assert_eq!(
        before.current.as_ref().unwrap().completeness_id,
        after.current.as_ref().unwrap().completeness_id
    );
    assert_eq!(before.history_count, after.history_count);
}

#[test]
fn evidence_completeness_negative_authority_guards() {
    assert!(WorkspaceEvidenceCompletenessService::attempt_execute().is_err());
    assert!(WorkspaceEvidenceCompletenessService::attempt_create_recommendation().is_err());
    assert!(WorkspaceEvidenceCompletenessService::attempt_estimate_completeness().is_err());
    assert!(WorkspaceEvidenceCompletenessService::attempt_repair_evidence().is_err());
    assert!(WorkspaceEvidenceCompletenessService::attempt_regenerate_snapshot().is_err());
    assert!(WorkspaceEvidenceCompletenessService::attempt_fabricate_completeness().is_err());
    assert!(WorkspaceEvidenceCompletenessService::attempt_fill_gaps().is_err());
    assert!(WorkspaceEvidenceCompletenessService::attempt_emit_command().is_err());
    assert!(CommandHandler::evidence_completeness_attempt_execute().is_err());
}

#[test]
fn evidence_completeness_capability_deny_without_write() {
    ...
    let err = crate::commands::pipeline::CommandPipeline::new(ctx)
        .execute_mutation(
            crate::commands::workspace_evidence_completeness::GenerateWorkspaceEvidenceCompleteness::new(
                ws,
            ),
        )
...
#[test]
fn explain_evidence_completeness_is_non_actionable() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    CommandHandler::generate_workspace_evidence_completeness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws.clone(),
    ...
    .unwrap();
    let explanation = CommandHandler::explain_evidence_completeness(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
        ws,
...
#[test]
fn evidence_completeness_history_is_non_actionable_evidence() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    seed_upstream_evidence(&kernel, &ws);
    let actor = ActorContext::local_user();
    ...
    let summary = CommandHandler::get_workspace_evidence_completeness_summary(
        &kernel,
        ActorContext::local_user(),
        IntentContext::user_request(),
...
#[test]
fn evidence_completeness_service_never_calls_foreign_generate() {
    let source = include_str!("../services/workspace_evidence_completeness.rs");
...
#[test]
fn no_execution_paths_in_evidence_completeness_service() {
    let source = include_str!("../services/workspace_evidence_completeness.rs");
...
#[test]
fn no_fabricated_completeness_without_upstream() {
    let kernel = WorkspaceKernel::initialize_in_memory().unwrap();
    let ws = seed_workspace(&kernel);
    let snap = CommandHandler::generate_workspace_evidence_completeness(
        &kernel,
...
#[test]
fn no_repair_ownership_in_completeness_service() {
    let source = include_str!("../services/workspace_evidence_completeness.rs");
...
#[test]
fn no_evidence_synthesis_paths_in_completeness_service() {
    let source = include_str!("../services/workspace_evidence_completeness.rs");
    assert!(!source.contains("TaskGraphService"));
    assert!(!source.contains("WorkspacePlanningService"));
    assert!(!source.contains("ExecutionLifecycleService"));
