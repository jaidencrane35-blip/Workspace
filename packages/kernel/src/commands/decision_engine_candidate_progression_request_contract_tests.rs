//! DecisionCandidateProgressionRequest — downstream consideration request only
...
use workspace_domain::{
    ...
    DecisionCandidateLifecycleIntegration, DecisionCandidateProgressionRequest,
...
fn selected_progression_input(
    candidate: DecisionCandidate,
) -> DecisionCandidateProgressionRequestInput {
...
/// CASE 1 — Selected candidate creates progression request.
#[test]
fn case1_selected_creates_progression_request() {
    let input = selected_progression_input(native_candidate_with_total("pr-1", 50));
    assert!(DecisionCandidateProgressionRequest::derive(&input).is_pending());
    let before = input.candidate.clone();
    let (request, unchanged) =
        DecisionCandidateProgressionRequest::try_request(&input, "downstream", "t-req")
            .expect("request");
    assert!(request.is_requested());
    assert!(request
        .request_id
        .starts_with(DecisionCandidateProgressionRequest::ID_PREFIX));
    assert_eq!(request.selection_id, input.selection.selection_id);
    assert_eq!(unchanged.outcome, before.outcome);
    assert!(request.assert_request_only().is_ok());
}

/// CASE 2 — Non-selected candidate blocked.
#[test]
fn case2_non_selected_blocked() {
    let mut input = selected_progression_input(native_candidate_with_total("pr-2", 50));
    input.selection.selection_state = DecisionCandidateSelection::STATE_AWAITING.into();
    input.selection.selected_at = None;
    assert!(DecisionCandidateProgressionRequest::derive(&input).is_blocked());
    assert!(DecisionCandidateProgressionRequest::try_request(&input, "x", "t").is_err());
}

/// CASE 3 — Invalid provenance blocked.
#[test]
fn case3_invalid_provenance_blocked() {
    let mut input = selected_progression_input(intake_created_candidate("pr-3"));
    input.candidate.package_seal_digest = None;
    assert!(DecisionCandidateProgressionRequest::derive(&input).is_blocked());
    assert!(DecisionCandidateProgressionRequest::try_request(&input, "x", "t").is_err());
}

/// CASE 4 — Withdrawn selection blocked.
#[test]
fn case4_withdrawn_selection_blocked() {
    let mut input = selected_progression_input(intake_created_candidate("pr-4"));
    input.intake_withdrawn_or_invalidated = true;
    input.selection.selection_state = DecisionCandidateSelection::STATE_WITHDRAWN.into();
    assert!(DecisionCandidateProgressionRequest::derive(&input).is_blocked());
    assert!(DecisionCandidateProgressionRequest::try_request(&input, "x", "t").is_err());
}

/// CASE 5 — Request does not execute.
#[test]
fn case5_request_does_not_execute() {
    let input = selected_progression_input(native_candidate_with_total("pr-5", 50));
    let (request, _) =
        DecisionCandidateProgressionRequest::try_request(&input, "downstream", "t-req").unwrap();
    assert!(!request.may_execute());
    assert_blocked(
        "execute",
        DecisionCandidateProgressionRequest::attempt_execute().map_err(KernelError::from),
    );
    let _ = CommandHandler::decision_engine_attempt_execute();
}

/// CASE 6 — Request does not call planner.
#[test]
fn case6_request_does_not_call_planner() {
    let input = selected_progression_input(native_candidate_with_total("pr-6", 50));
    let (request, _) =
        DecisionCandidateProgressionRequest::try_request(&input, "downstream", "t-req").unwrap();
    assert!(!request.planner_invoked && !request.may_invoke_planner());
    assert!(request.handoff_command.is_none());
    assert!(request.attempt_invoke_planner().is_err());
}

/// CASE 7 — Request does not create goals/intents.
#[test]
fn case7_request_does_not_create_goals_intents() {
    let input = selected_progression_input(native_candidate_with_total("pr-7", 50));
    let (request, _) =
        DecisionCandidateProgressionRequest::try_request(&input, "downstream", "t-req").unwrap();
    assert!(!request.creates_goal && !request.may_create_goal());
    assert!(!request.creates_intent && !request.may_create_intent());
    assert!(request.attempt_create_goal().is_err());
    assert!(request.attempt_create_intent().is_err());
}

/// CASE 8 — Request does not call Gateway.
#[test]
fn case8_request_does_not_call_gateway() {
    let input = selected_progression_input(native_candidate_with_total("pr-8", 50));
    let (request, _) =
        DecisionCandidateProgressionRequest::try_request(&input, "downstream", "t-req").unwrap();
    assert!(!request.may_invoke_gateway());
    assert!(request.attempt_invoke_gateway().is_err());
}

/// CASE 9 — RE remains unchanged.
#[test]
fn case9_re_unchanged() {
    let input = selected_progression_input(intake_created_candidate("pr-9"));
    let (request, _) =
        DecisionCandidateProgressionRequest::try_request(&input, "downstream", "t-req").unwrap();
    assert!(!request.mutates_recommendation_engine);
    assert!(!request.ownership_transferred);
}

/// CASE 10 — Native and recommendation-derived remain distinguishable.
#[test]
fn case10_origins_distinguishable() {
    let native = DecisionCandidateProgressionRequest::try_request(
        &selected_progression_input(native_candidate_with_total("pr-10n", 50)),
        "downstream",
        "t-req",
    )
    .unwrap()
    .0;
    let intake = DecisionCandidateProgressionRequest::try_request(
        &selected_progression_input(intake_created_candidate("pr-10i")),
        "downstream",
        "t-req",
    )
    .unwrap()
...
fn case11_synthesis_unchanged() {
    let native = native_candidate_with_total("pr-11", 10);
    ...
    let input = selected_progression_input(native);
    let (request, unchanged) =
        DecisionCandidateProgressionRequest::try_request(&input, "downstream", "t-req").unwrap();
    let with = base.with_progression_requests(vec![request]);
    ...
    assert!(!with.progression_requests[0].mutates_candidate_outcome);
}
