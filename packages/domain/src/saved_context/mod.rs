...
pub const SAVED_CONTEXT_SCOPE_ID: &str = "saved-context-scope-v1";
...
mod tests {
    ...
    fn rejects_consent_given_for_a_different_scope() {
        let error = request("Tuesday review", "saved-context-scope-v0")
            .validate()
            .unwrap_err();
        assert_eq!(
            error,
            SavedContextError::ConsentStale {
                approved: "saved-context-scope-v0".into(),
    ...
    fn the_scope_declares_the_window_title_caveat_it_cannot_avoid() {
        // Window titles routinely name the page or file behind them. The preview
        // has to say so, because the capture cannot avoid it.
        let scope = SavedContextCaptureScope::current();
        let titles = scope
            .captured
            .iter()
