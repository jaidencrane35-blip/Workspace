...
-- Analysis artefacts only — never executes, approves, decides, predicts, or mutates.

CREATE TABLE IF NOT EXISTS workspace_contextual_understanding_snapshots (
...
CREATE INDEX IF NOT EXISTS idx_workspace_contextual_understanding_snapshots_workspace
    ON workspace_contextual_understanding_snapshots (workspace_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_workspace_contextual_understanding_snapshots_status
    ON workspace_contextual_understanding_snapshots (workspace_id, status);

CREATE TABLE IF NOT EXISTS workspace_contextual_understanding_history (
    id TEXT PRIMARY KEY NOT NULL,
    workspace_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_workspace_contextual_understanding_history_workspace
    ON workspace_contextual_understanding_history (workspace_id, recorded_at DESC);
